...
local TableLayer = require("game.huaihua.TableLayer")
local HHViewManager = require("game.huaihua.HHViewManager")
local APPNAME = 'huaihua'
local HHGameLayer = class("HHGameLayer",function()
    return ccui.Layout:create()
end)

function HHGameLayer:create(...)
    local view = HHGameLayer.new()
    view:onCreate(...)
...
function HHGameLayer:onEnter()
    ...
    EventMgr:registListener('HHhideEndLayer',self,self.HHhideEndLayer)
...
function HHGameLayer:onExit()
    ...
    EventMgr:unregistListener('HHhideEndLayer',self,self.HHhideEndLayer)
...
function HHGameLayer:onCleanup()

end

function HHGameLayer:onCreate(...)
    self:startGame(...)
end

function HHGameLayer:startGame(...)
    ...
    local csb = cc.CSLoader:createNode("HHGameLayer.csb")
    self:addChild(csb)
...
function HHGameLayer:loadingPlayback()
    if GameCommon.tableConfig.nTableType ~= TableType_Playback then
        return
    end
...
function HHGameLayer:AnalysisPlaybackData()
...
function HHGameLayer:SUB_GR_MATCH_TABLE_ING(event)
    local data = event._usedata
    self:startGame(UserData.User.userID, data)
end

function HHGameLayer:EVENT_TYPE_NET_RECV_MESSAGE(event)
...
function HHGameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    local _tagMsg = {}
    ...
    if mainCmdID == NetMsgId.MDM_GR_USER then   
       if subCmdID == NetMsgId.SUB_GR_USER_READY then
        ...
        elseif subCmdID == NetMsgId.SUB_GR_TABLE_STATUS then 
            ...
            Text_des_numb:setString(string.format("局数:%d/%d",randCeil,randFloor))
            return true
        ...
        elseif subCmdID == NetMsgId.SUB_GR_DISMISS_TABLE_STATE then
            ...
            HHViewManager.openDimissTable(data)
...
function HHGameLayer:EVENT_TYPE_CACEL_MESSAGE_BLOCK(event)
    self.isRunningActions = false
    self:updatehandplate()
end

--@ fux 解散房间成功
function HHGameLayer:disRoomSuccess( pBuffer )
...
function HHGameLayer:conformEnd()
    if not self.pBuffer then
        print('==================>>>结算界面pBuff为nil')
        return
    end
    performWithDelay(self.root,function( ... )
        local path = self:requireClass('HHGameRoomEnd')
...
--消息队列
function HHGameLayer:update(delta)
...
--消息执行
function HHGameLayer:OnGameMessageRun(_tagMsg)
            ...
            if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.huaihua.44.GameEndLayer"):create(pBuffer))
                    end
                    end),
...
function HHGameLayer:HHhideEndLayer( event )
...
function HHGameLayer:EVENT_TYPE_OPERATIONAL_OUT_CARD(event)
    local data = event._usedata
...
function HHGameLayer:SUB_GR_USER_ENTER(event)
    local data = event._usedata
    self:startGame(UserData.User.userID, data)
end

--更新玩家信息
function HHGameLayer:updatePlayerInfo()
...
function HHGameLayer:updatePlayerlScore()
    if GameCommon.gameConfig == nil then
        return
    end
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
...
function HHGameLayer:updatehandplate()
    if  GameCommon.gameConfig == nil then
        return
    end
    --@desc fux 张数
...
function HHGameLayer:updateBankerUser()
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        local wChairID = i-1
        local viewID = GameCommon:getViewIDByChairID(wChairID)
        local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_player%d",viewID))
...
function HHGameLayer:updatePlayerHuXi(wChairID)
    self.tableLayer:updatePlayerHuXi(wChairID)
end

function HHGameLayer:updatePlayerReady()
    if GameCommon.gameConfig == nil then
        return
    end
    local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,"Panel_player")
...
function HHGameLayer:updatePlayerOnline()
    if GameCommon.gameConfig == nil then
        return
    end
    local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,"Panel_player")
...
function HHGameLayer:updatePlayerPosition()
...
function HHGameLayer:requireClass( name )
