...
--    self.meChairID = 0
    ...
    self.language = cc.UserDefault:getInstance():getIntegerForKey('HHlanguage', 0)  	--语言 0-普通话 1-方言
end
...
function GameCommon:GetCardHand(data,isSmall)
    ...
    elseif data <= 10 then
        if isSmall then
            imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data,'m'))
        else
            imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data,'x'))
        end
    elseif data > 10 then
        if isSmall then
            imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data-16,'mm'))
        else
            imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data-16,'d'))
        end
    else
        assert(false)
    end
...
function GameCommon:getSendOrOutCard(data, isSendCard)
    ...
    elseif data==33 then
        ...
        -- end
    elseif data <= 10 then
        imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data,'dx'))
    elseif data > 10 then
        imgCard = ccui.ImageView:create(self.cardPath(cardIndex,data-16,'dd'))
    else
        assert(false)
    end
...
function GameCommon:getDiscardCardAndWeaveItemArray(data)
    ...
    elseif data <= 10 then
        imgCard =  ccui.ImageView:create(self.cardPath(cardIndex,data,'ss'))
    elseif data > 10 then
        imgCard =  ccui.ImageView:create(self.cardPath(cardIndex,data-16,'s'))
    else
        assert(false)
    end
    imgCard.data = data
...
function GameCommon.cardPath( index,numb ,name)
    return string.format("huaihua/card/card%d/%s%d.png",index,name,numb)
end
