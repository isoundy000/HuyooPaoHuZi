...
function GameOperation:onEnter()
	self.isFast = cc.UserDefault:getInstance():getBoolForKey('HHisFastEat', true)	
...
function GameOperation:onCreate(opType, cbOperateCode, cbOperateCard, cbCardIndex, cbSubOperateCode)
	...
	local csb = cc.CSLoader:createNode("HHOperationLayer.csb")
	self:addChild(csb)
	...
	elseif opType == 2 then
		...
		if(Bit:_and(cbOperateCode, GameCommon.ACK_BIHU) ~= 0) then
			...
			item:loadTextures("huaihua/ui/operate/hu.png", "huaihua/ui/operate/hu.png", "huaihua/ui/operate/hu.png")
			item:setPressedActionEnabled(true)
...
-- armature:setPosition(armature:getParent():getContentSize().width/2,armature:getParent():getContentSize().height/2)
			...
			if(Bit:_and(cbOperateCode, GameCommon.ACK_CHI) ~= 0) or(Bit:_and(cbOperateCode, GameCommon.ACK_CHI_EX) ~= 0) then
				...
				local item = self.Button_operation:clone()
				item:ignoreContentAdaptWithSize(true)
				item:loadTextures("huaihua/ui/operate/chi.png", "huaihua/ui/operate/chi.png", "huaihua/ui/operate/chi.png")
				item:setPressedActionEnabled(true)
				uiPanel_operation:addChild(item)
				item:addTouchEventListener(function(sender, event)
					if event == ccui.TouchEventType.ended then
			...
			if Bit:_and(cbOperateCode, GameCommon.ACK_PENG) ~= 0 then
				local item = self.Button_operation:clone()
				item:ignoreContentAdaptWithSize(true)
				item:loadTextures("huaihua/ui/operate/peng.png", "huaihua/ui/operate/peng.png", "huaihua/ui/operate/peng.png")
				item:setPressedActionEnabled(true)
				uiPanel_operation:addChild(item)
				item:addTouchEventListener(function(sender, event)
					if event == ccui.TouchEventType.ended then
			...
			if cbOperateCode ~= GameCommon.ACK_PAO then
				local item = self.Button_operation:clone()
				item:ignoreContentAdaptWithSize(true)
				item:loadTextures("huaihua/ui/operate/guo.png", "huaihua/ui/operate/guo.png", "huaihua/ui/operate/guo.png")
				item:setPressedActionEnabled(true)				
				uiPanel_operation:addChild(item)
				item:addTouchEventListener(function(sender, event)
					if event == ccui.TouchEventType.ended then
...
function GameOperation:dealEat()
	if self.isFast then --开启快速吃牌
		...
		if not self.clickRoot then
			self.clickRoot = self:createChild(self.cbCardIndex, nil,self.isLeft)
		end
	end
end
...
function GameOperation:fastEat(chidCardTable)
	...
	if not self.clickRoot then
		self.clickRoot = self:createChild(self.cbCardIndex, nil,true)
	end
end
...
--创建一个子节点
function GameOperation:createChild(chidCardTable, front,isLeft)
	...
	if childCount > 0 then --有child 可生成
        ...
        local bgSize = cc.size(childCount *65 + (childCount - 1) * 14, 180)
        local rootSize = cc.size(bgSize.width + 10 , 185)
		...
		item_list:setPosition(rootSize.width / 2,rootSize.height/2)
		--添加button
			...
			button:addTouchEventListener(function(sender, event)
				if event == ccui.TouchEventType.ended then
					...
					if childCount > 0 then
						...
						local child = self:createChild(cbCardTable, image_root,isLeft)
...
function GameOperation:changeLayout( ... )
	...
	if self.layout == 0 then --居右
		...
		local y = visibleSize.height - 200
