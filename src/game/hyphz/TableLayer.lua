...
local GameOperation = require("game.hyphz.GameOperation")
...
local cardtype = cc.UserDefault:getInstance():getIntegerForKey('CDpaiSize', 1)
local CardSize = cc.size(97, 111) --CardSizeArr[cardtype]
local SmallCardSize = cc.size(32, 38)
local IsOpenTin = cc.UserDefault:getInstance():getIntegerForKey('HYTPTS', 1)
local CtlSpeed = cc.UserDefault:getInstance():getIntegerForKey('HYSpeed', 2)
...
function TableLayer:setWeaveItemArray(wChairID, bWeaveItemCount, WeaveItemArray, location)
	...
	local cardScale = 1.2
	-- local setScale = cc.UserDefault:getInstance():getIntegerForKey('HYZSize', 1)
	-- if setScale ~= 1 then
	-- 	cardScale = 1.4
	-- end
	local cardSpace = 0
	local cardWidth = SmallCardSize.width * cardScale 
	local cardHeight = SmallCardSize.height * cardScale + cardSpace
	local stepX = 1
	local stepY = 1
	local maxRow = 7
...
function TableLayer:setDiscardCard(wChairID, bDiscardCardCount, bDiscardCard,bOutCardMark)
	...
	-- end
	local cardWidth = SmallCardSize.width * cardScale
	local cardHeight = SmallCardSize.height * cardScale
	local stepX = cardWidth
	local stepY = cardHeight - 5
	local maxRow = 6
	...
	for i = 1, bDiscardCardCount do
		...
		if anchorPoint.y == 1 then
			...
			card:setLocalZOrder(bDiscardCardCount-i)
		end
...
function TableLayer:initUI()
	...
	--UI层
	local uiButton_menu = ccui.Helper:seekWidgetByName(self.root, "Button_setmenu")
	local Image_function = ccui.Helper:seekWidgetByName(self.root, "Image_function")
	local Panel_menu = ccui.Helper:seekWidgetByName(self.root, "Panel_menu")
	local Button_tuichu = ccui.Helper:seekWidgetByName(self.root, "Button_tuichu")
	Panel_menu:setSwallowTouches(false)
	Image_function:setVisible(false)
	Common:addTouchEventListener(uiButton_menu, function()
		if Image_function:isVisible() then
			Image_function:setVisible(false)
		else
			Image_function:setVisible(true)
		end
	end)

	Panel_menu:addTouchEventListener(function(sender, event)
		if event == ccui.TouchEventType.ended then
			Image_function:setVisible(false)
		end
	end)

	local Image_bg = ccui.Helper:seekWidgetByName(self.root,"Image_bg")
	local Button_out = ccui.Helper:seekWidgetByName(self.root,"Button_out")
	local Button_jiesan = ccui.Helper:seekWidgetByName(self.root,"Button_jiesan")
	Common:addTouchEventListener(ccui.Helper:seekWidgetByName(self.root, "Button_skin"), function()
		local UserDefault_ZiPaipaizhuo = cc.UserDefault:getInstance():getIntegerForKey(Default.UserDefault_ZiPaipaizhuo, 1)
		UserDefault_ZiPaipaizhuo = UserDefault_ZiPaipaizhuo + 1
		if UserDefault_ZiPaipaizhuo < 1 or UserDefault_ZiPaipaizhuo > 4 then
			UserDefault_ZiPaipaizhuo = 1
		end
		cc.UserDefault:getInstance():setIntegerForKey(Default.UserDefault_ZiPaipaizhuo, UserDefault_ZiPaipaizhuo)
		Image_bg:loadTexture(string.format('hyphz/table/pz_cl_bk_%d.jpg', UserDefault_ZiPaipaizhuo))
		local path1 = string.format('hyphz/table/btn_fh_normal_%d.png', UserDefault_ZiPaipaizhuo-1)
		local path2 = string.format('hyphz/table/btn_jsfj_normal_%d.png', UserDefault_ZiPaipaizhuo-1)
		local path3 = string.format('hyphz/table/btn_tcfj_normal_%d.png', UserDefault_ZiPaipaizhuo-1)
		Button_out:loadTextures(path1, path1, path1)
		Button_jiesan:loadTextures(path2, path2, path2)
		Button_tuichu:loadTextures(path3,path3,path3)

		local idx = UserDefault_ZiPaipaizhuo
		if idx > 3 then
			idx = 3
		end
		local Image_waterbg = ccui.Helper:seekWidgetByName(self.root,"Image_waterbg")
		Image_waterbg:loadTexture(string.format('hyphz/table/pz_cl0%d.png', idx))
	end)
	...
	Common:addTouchEventListener(Button_jiesan, function()
	...
	Common:addTouchEventListener(ccui.Helper:seekWidgetByName(self.root, "Button_set"), function()
		local path = self:requireClass('HYSetting')
	...
	local function onEventExpression(sender, event)
		if event == ccui.TouchEventType.ended then
			Common:palyButton()
			local child = self:getChildByName('HYChat')
			if child and child:getName() == 'HYChat' then
				child:setVisible(true)
				return true
			end
			local path = self:requireClass('HYChat')
			local box = require("app.MyApp"):create():createGame(path)
			box:setName('HYChat')
			self:addChild(box)
	...
	local Button_tuichu = ccui.Helper:seekWidgetByName(self.root, "Button_tuichu")
	Common:addTouchEventListener(Button_tuichu, function()
	...
	if GameCommon.tableConfig.nTableType > TableType_GoldRoom then
		...
		elseif GameCommon.tableConfig.wCurrentNumber > 0 then
			...
			local Button_tuichu = ccui.Helper:seekWidgetByName(self.root, "Button_tuichu")
			Button_tuichu:setVisible(false)
		end
...
-- uiButton_Invitation:addChild(waitArmature)   
	...
	local function onEventReset(sender, event)
		if event == ccui.TouchEventType.ended then
			Common:palyButton()
			require("common.SceneMgr"):switchOperation(require("app.MyApp"):create():createGame("game.hyphz.HYLocationLayer"))
		end
	end
...
function TableLayer:showPlayerInfo(infoTbl)       -- 查看玩家信息
	Common:palyButton()
	require("common.SceneMgr"):switchOperation(require("app.MyApp"):create(infoTbl, self):createGame("game.hyphz.HYPersonInfor"))
...
function TableLayer:initTableViewInfo()
	...
	-- bgNum = cc.UserDefault:getInstance():getIntegerForKey('HYzipaiBg', 1)
	-- if bgNum < 1 or bgNum > 4 then
	-- 	bgNum = 1
	-- end
	-- uiPanel_bg:loadTexture(string.format("hyphz/table/pz_cl_bk_%d.jpg", bgNum))
	...
	--卡牌大小
	-- cardtype = cc.UserDefault:getInstance():getIntegerForKey('CDpaiSize', 1)
	-- CardSize = CardSizeArr[cardtype]
	
	IsOpenTin = cc.UserDefault:getInstance():getIntegerForKey('HYTPTS', 1)
	if IsOpenTin == 1 then
		local Image_tp = ccui.Helper:seekWidgetByName(self.root, "Image_tp")
		Image_tp:setVisible(false)
	end
	
	CtlSpeed = cc.UserDefault:getInstance():getIntegerForKey('HYSpeed', 2)
...
function TableLayer:fourTableLayout()
	...
	Panel_discardCard1:setPosition(size.width * 0.107, size.height * 0.29)
	
	local Panel_discardCard2 = Panel_discardCard:getChildByName("Panel_discardCard2")
	Panel_discardCard2:setAnchorPoint(0, 1)
	Panel_discardCard2:setPosition(size.width * 0.107, size.height * 0.69)
	
	local Panel_discardCard3 = Panel_discardCard:getChildByName("Panel_discardCard3")
	Panel_discardCard3:setAnchorPoint(1, 1)
	Panel_discardCard3:setPosition(size.width * 0.89, size.height * 0.69)
	
	local Panel_discardCard4 = Panel_discardCard:getChildByName("Panel_discardCard4")
	Panel_discardCard4:setAnchorPoint(1, 0)
	Panel_discardCard4:setPosition(size.width * 0.89, size.height * 0.29)
	...
	Panel_weaveItemArray1:setPosition(size.width * 0.107, size.height * 0.043)
	...
	Panel_weaveItemArray2:setPosition(size.width * 0.107, size.height * 0.704)
	...
	Panel_weaveItemArray3:setPosition(size.width * 0.89, size.height * 0.704)
	...
	Panel_weaveItemArray4:setPosition(size.width * 0.89, size.height * 0.043)
...
function TableLayer:playSkelStartToEndPos(sChairID, eChairID, index)
	local open = cc.UserDefault:getInstance():getIntegerForKey('HYHDDH', 1) 
	self.isOpen = open == 2
...
function TableLayer:playSketlAnim(sChairID, eChairID, index,indexEx)
	...
	local Animation = require("game.hyphz.Animation")
...
function TableLayer:setTablePlayWayInfo()
	...
	Text_node:setString(string.format("房间:%d 第%d/%d局", roomId, curJuShu, maxJuShu))

	local playwayDes = GameDesc:getGameDesc(GameCommon.tableConfig.wKindID, GameCommon.gameConfig, GameCommon.tableConfig)
	local Text_playwayshow = GameCommon:seekWidgetByNameEx(self.root, "Text_playwayshow")
	Text_playwayshow:setString(playwayDes)
end
...
function TableLayer:setTPCardFlag(card, value)
	...
	if self.dragTPData and self.dragTPData.cbCardIndex[valueIdx] > 0 and IsOpenTin ~= 1 then
