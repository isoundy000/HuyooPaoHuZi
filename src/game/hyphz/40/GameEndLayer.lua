...
function GameEndLayer:showMingTang(pBuffer)
	...
	if Bit:_and(pBuffer.wType, self.PHZ_HT_HongHu) ~= 0 then
	   if GameCommon.gameConfig.bCardCount21 == 1 and wHZCount >= 13 then
            local str = string.format("红胡:%d番", 4)
            self:insertInfo(str)
       elseif GameCommon.gameConfig.bCardCount21 == 1 and wHZCount >= 10 then
            local str = string.format("红胡:%d番", 2)
            self:insertInfo(str) 
       elseif GameCommon.gameConfig.bCardCount21 == 0 and wHZCount >= 10 then
            local str = string.format("红胡:%d番", 4)
            self:insertInfo(str) 
       elseif GameCommon.gameConfig.bCardCount21 == 0 and wHZCount >= 8 then
            local str = string.format("红胡:%d番", 2)
            self:insertInfo(str) 
	   end
	...
	if Bit:_and(pBuffer.wType, self.PHZ_HT_DaZiHu) ~= 0 then
		local str = string.format("大字胡:%d番", 2)
		self:insertInfo(str)
	end
	--小字胡判断
	if Bit:_and(pBuffer.wType, self.PHZ_HT_XiaoZiHu) ~= 0 then
		local str = string.format("小字胡:%d番", 2)
		self:insertInfo(str)
	end
	--天胡判断
	...
	if Bit:_and(pBuffer.wType, self.PHZ_HT_DuiDuiHu) ~= 0 then
		local str = string.format("碰碰胡:%d番", 2)
		self:insertInfo(str)
	end
	--飘胡判断
	if Bit:_and(pBuffer.wType, self.PHZ_HT_PiaoHu) ~= 0 then
		local str = string.format("飘胡:%d胡息", 6)
		self:insertInfo(str)
	end
