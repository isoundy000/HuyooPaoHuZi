...
function GameEndLayer:updatePlayerInfo(pBuffer)
	...
	for key, var in pairs(GameCommon.player) do
		...
		elseif pBuffer.wWinUser == 65535 and var.wChairID == GameCommon.meChairID then
			item = itemCache[1]
			local dwGold = Common:itemNumberToString(pBuffer.lGameScore[var.wChairID + 1])   
			self.text_total:setString('共计:' ..tostring(dwGold))
		else
		...
		if wKindID == 39 then
			self:tiLong(pBuffer.stuTingLong[var.wChairID], var.szNickName)
		end
...
function GameEndLayer:onCreate(pBuffer)
	...
	local wKindID = GameCommon.tableConfig.wKindID
	if wKindID == 39 then
		self.Panel_tilong = ccui.Helper:seekWidgetByName(self.root, "Panel_tilong")
		self.Panel_tilong:setVisible(true)
		self.template = ccui.Helper:seekWidgetByName(self.root, "Panel_tilongtemplate")
		self.ListView_tilong = self.Panel_tilong:getChildByName('ListView_tilong')
		local uiListView_weave = ccui.Helper:seekWidgetByName(self.root, "ListView_weave")
		uiListView_weave:setPosition(TiLongPos[1])
	end
	...
	if pBuffer.wWinUser == 65535 then
		textureName = "cdzipai/ui/title3.png"
	else
...
function GameEndLayer:showMingTang(pBuffer)
	...
	self.XiaoKaHu				= 8388608              --小卡胡
	self.DaKaHu					= 16777216             --大卡胡
	self.WuHu					= 33554432             --无胡
	self.JuShou				= 67108864 --举手做声
	...
	for i = 1, 2 do
		if pBuffer.fanXing[i].cbShengCard ~= 0 then
			if GameCommon.gameConfig.FanXing.bType == 1 or GameCommon.gameConfig.FanXing.bType == 2 then
				...
				self:updateFanXing('翻省', str, pBuffer.fanXing[i].cbShengCard)
				
			elseif GameCommon.gameConfig.FanXing.bType == 3 then
				local str = string.format("+%d", pBuffer.fanXing[i].cbShengCout)
				
				self:updateFanXing('跟省', str, pBuffer.fanXing[i].cbShengCard)
	...
	--举手
	if Bit:_and(pBuffer.wType, self.JuShou) ~= 0 then
		local str = string.format("举手:%d番", 2)
		self:insertInfo(str)
	end
end
...
--提龙
function GameEndLayer:tiLong(tiLongData, name)
	if not tiLongData then
		return
	end
	local item = self.template:clone()
	local name_ = item:getChildByName('name')
	local small_ = item:getChildByName('small')
	local big_ = item:getChildByName('big')
	self:setTextColor(name_)
	self:setTextColor(small_)
	self:setTextColor(big_)
	name_:setString(name)
	big_:setString(tiLongData.cbCountBig)
	small_:setString(tiLongData.cbCountSmall)
	self.ListView_tilong:pushBackCustomItem(item)
end
