...
function GameEndLayer:showMingTang(pBuffer)
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_HONGDUI) ~= 0 then
    	local str = string.format("红对胡:%d番", 6)
	self:insertInfo(str)
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_JIAHONGDUI) ~= 0 then
    	local str = string.format("夹红胡:%d番", 8)
	self:insertInfo(str)
    elseif Bit:_and(pBuffer.wType,self.PHZ_HT_HONGWU) ~= 0 then    --红乌
    	local str = string.format("红乌:%d番", 4)
	self:insertInfo(str)
    end
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_WUDUI) ~= 0 then
    	local str = string.format("乌对胡:%d番", 8)
	self:insertInfo(str)
     elseif Bit:_and(pBuffer.wType,self.PHZ_HT_HEIWU) ~= 0 then    --黑胡
    	local str = string.format("黑胡:%d番", 4)
	self:insertInfo(str)
     end
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_DUIDUIHU) ~= 0 then
    	local str = string.format("对对胡:%d番", 4)
	self:insertInfo(str)
    end
end
