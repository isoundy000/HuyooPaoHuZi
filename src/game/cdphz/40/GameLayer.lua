...
function GameLayer:OnGameMessageRun(_tagMsg)
	...
	elseif mainCmdID == NetMsgId.MDM_GF_GAME then
		if subCmdID == NetMsgId.SUB_S_GAME_START then
			...
			if GameCommon.gameConfig.bCardCount21 == 1 then
				maxHanCardRow = 9
			end 
			...
			if GameCommon.gameConfig.bCardCount21 == 1 then 
				...
				if  GameCommon.gameConfig.bDeathCard == 1 then
                	self.tableLayer:updateLeftCardCount(80 - GameCommon.gameConfig.bPlayerCount * 14-20 - 1, true)
            	else
		...
		elseif subCmdID == NetMsgId.SUB_S_CLIENTERROR then
			...
			self.tableLayer:setHandCard(wChairID, GameCommon.player[wChairID].bUserCardCount, pBuffer.cbCardIndex, 7, 0)
			...
			if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
				...
				cc.CallFunc:create(function(sender, event)
					if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
						...
						uiPanel_end:addChild(require("game.cdphz.40.GameEndLayer"):create(pBuffer))
					end
				end),
	...
	elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
		if subCmdID == NetMsgId.SUB_GF_SCENE then
			...
			self.tableLayer:setHandCard(wChairID, pBuffer.bUserCardCount[wChairID + 1], pBuffer.cbCardIndex, 7, 0, true)
			self.tableLayer:showHandCard(wChairID, 0)
			self:updateBankerUser()
			...
			if i - 1 ~= GameCommon:getRoleChairID() then
				for j = 1, GameCommon.player[i - 1].bWeaveItemCount do
					if GameCommon.player[i - 1].WeaveItemArray[j].cbWeaveKind == GameCommon.ACK_WEI and GameCommon.player[i - 1].WeaveItemArray[j].cbCenterCard == cbCardData then
						...
						end,function() self.tableLayer:showHandCard(wChairID, 0) end)
						return
					end
				end
			end
