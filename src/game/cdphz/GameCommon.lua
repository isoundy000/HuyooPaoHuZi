...
--    self.meChairID = 0
    ...
    self.huangfanNum = 0
    self.language = cc.UserDefault:getInstance():getIntegerForKey('CDvolumeSelect', 0)
...
function GameCommon:GetCardHand(data)
    ...
    local imgCard = nil
    local cardIndex = nil
    local cardBgIndex = cc.UserDefault:getInstance():getIntegerForKey('CDpaiSize', 2)

    if self:isSelectCDGameType() then
        cardIndex = cc.UserDefault:getInstance():getIntegerForKey('CDzipaiSelect', 3)
    else
        cardIndex = cc.UserDefault:getInstance():getIntegerForKey('HYzipaiSelect', 1)
    end
    
    if data == 0 then
        imgCardFrame = ccui.ImageView:create(string.format("cdzipai/card/small_back%d.png",cardBgIndex))
    elseif data==33 then
        -- if cardIndex == 0 then
        ...
        -- end
    elseif data <= 10 then
        imgCardFrame = ccui.ImageView:create(string.format("cdzipai/card/small_face%d.png", cardBgIndex))
        imgCard = ccui.ImageView:create(string.format("cdzipai/card/cards%d_%ds.png",cardIndex, data))
        imgCardFrame:addChild(imgCard)
        local size = imgCardFrame:getContentSize()
        imgCard:setPosition(size.width * 0.5, size.height * 0.65)
    elseif data > 10 then
        imgCardFrame = ccui.ImageView:create(string.format("cdzipai/card/small_face%d.png", cardBgIndex))
        imgCard = ccui.ImageView:create(string.format("cdzipai/card/cards%d_%db.png",cardIndex, data-16))
        imgCardFrame:addChild(imgCard)
        local size = imgCardFrame:getContentSize()
        imgCard:setPosition(size.width * 0.5, size.height * 0.65)
    else
...
function GameCommon:getSendOrOutCard(data, isSendCard)
    ...
    local imgBg = nil
    local proportion = {0.8, 0.2}
    if isSendCard == true then
        imgBg = ccui.ImageView:create("cdzipai/card/face1.png")
        proportion = {0.75, 0.25}
    else
        imgBg = ccui.ImageView:create("cdzipai/card/face.png")
    end
    ...
    local imgCard = nil
    if data == 0 then
        imgBg = ccui.ImageView:create("cdzipai/card/back.png")
    elseif data==33 then
        -- if cardIndex == 0 then
    ...
    elseif data <= 10 then
        ...
        imgCard:setScale(scalesize)
        local size = imgBg:getContentSize()
        imgCard:setPosition(size.width * 0.5, size.height * proportion[1])
        local imgCardDown = imgCard:clone()
        imgBg:addChild(imgCardDown)
        imgCardDown:setPosition(size.width * 0.5, size.height * proportion[2])
        imgCardDown:setFlippedY(true)
        imgCardDown:setFlippedX(true)
    elseif data > 10 then
        imgCard = ccui.ImageView:create(string.format("cdzipai/card/cards%d_%db.png",cardIndex, data-16))
        imgBg:addChild(imgCard)
        imgCard:setScale(scalesize)
