...
function GameEndLayer:showMingTang(pBuffer)
	...
	if wHZCount >= 10 then
		...
		if Bit:_and(pBuffer.wType, self.PHZ_HT_DUIDUIHU) ~= 0 then
			...
			local str = string.format("红对胡:%d番", 7 + wHZCount - 10)
			self:insertInfo(str)
		else    --红胡
			local str = string.format("红胡:%d番", 2 + wHZCount - 10)
			self:insertInfo(str)
		end
	end
	...
	if Bit:_and(pBuffer.wType, self.PHZ_HT_HEIWU) ~= 0 then
		--乌对胡
		if Bit:_and(pBuffer.wType, self.PHZ_HT_DUIDUIHU) ~= 0 then
			local str = string.format("乌对胡:%d番", 10)
			self:insertInfo(str)
		else    --黑胡
	...
	if Bit:_and(pBuffer.wType, self.PHZ_HT_DUIDUIHU) ~= 0 and wHZCount < 10 and Bit:_and(pBuffer.wType, self.PHZ_HT_HEIWU) == 0 then
		local str = string.format("对对胡:%d番", 5)
			self:insertInfo(str)
	end
end
