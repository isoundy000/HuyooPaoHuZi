...
local TableLayer = require("game.cdphz.TableLayer")
...
function GameLayer:onEnter()
	...
	EventMgr:registListener(EventType.CHECK_TING_CARD_TIPS, self, self.CHECK_TING_CARD_TIPS)
...
function GameLayer:onExit()
	...
	EventMgr:unregistListener(EventType.CHECK_TING_CARD_TIPS, self, self.CHECK_TING_CARD_TIPS)
...
function GameLayer:startGame(...)
	...
	local csb = cc.CSLoader:createNode("CDGameLayer.csb")
	self:addChild(csb)
...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
	...
	if mainCmdID == NetMsgId.MDM_GR_USER then
		...
		elseif subCmdID == NetMsgId.SUB_GR_USER_OFFLINE then
			...
			local dwUserID = luaFunc:readRecvDWORD()
			local wChairID = luaFunc:readRecvWORD()
			GameCommon.player[wChairID].cbOnline = 6
			self:updatePlayerOnline()
			return true
		...
		elseif subCmdID == NetMsgId.SUB_GR_TABLE_STATUS then
			...
			-- Text_roomInfo:setString(string.format("房间:%d 第%d/%d局", roomId, randCeil, randFloor))
			self.tableLayer:setTablePlayWayInfo()
		...
		elseif subCmdID == NetMsgId.SUB_GR_DISMISS_TABLE_STATE then
			...
			if self:getChildByName('CDDisMissTable') then
				self:getChildByName('CDDisMissTable'):removeFromParent()
			end
			self.tableLayer:setDisMissData(data)
			if not self:checkWhoRefuse(data) then
		...
		elseif subCmdID == NetMsgId.SUB_GR_SEND_CHAT then
			...
			EventMgr:dispatch('SUB_GR_SEND_CHAT', _tagMsg.pBuffer)
			return
	...
	elseif mainCmdID == NetMsgId.MDM_GF_GAME then
		if subCmdID == NetMsgId.RET_SC_GAME_CONFIG then
			...
			-- local Text_playWayDes = GameCommon:seekWidgetByNameEx(self.root,"Text_playWayDes")
			-- local strdes = GameDesc:getGameDesc(GameCommon.tableConfig.wKindID,GameCommon.gameConfig,GameCommon.tableConfig)
			-- strdes = string.gsub(strdes, '/', ' ')
			-- Text_playWayDes:setString(strdes)
			self.tableLayer:setTablePlayWayInfo()
			return true
		...
		elseif subCmdID == NetMsgId.SUB_S_GAME_END then
			...
			print('----SUB_S_GAME_END:',GameCommon.huangfanNum)
		...
		elseif subCmdID == NetMsgId.SUB_GF_USER_EXPRESSION then
			...
			EventMgr:dispatch('SUB_GF_USER_EXPRESSION', _tagMsg.pBuffer)			
			return true
...
function GameLayer:OnGameMessageRun(_tagMsg)
	...
	if mainCmdID == NetMsgId.MDM_GR_USER then
		if subCmdID == NetMsgId.SUB_GR_GAME_STATISTICS then
			self:removeAllChildren()
			local layer = require("game.cdphz.FriendsRoomEndLayer"):create(pBuffer)
			self:addChild(layer)
		else
	...
	elseif mainCmdID == NetMsgId.MDM_GF_GAME then
		if subCmdID == NetMsgId.SUB_S_GAME_START then
			...
			print('-----------黄番次数:',GameCommon.huangfanNum)
			...
			if pBuffer.cbBeginCardData ~= nil and pBuffer.cbBeginCardData ~= 0 and(GameCommon.tableConfig.nTableType ~= TableType_Playback or wChairID + 1 == GameCommon.gameConfig.bPlayerCount) then
			...
			if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
				...
				cc.CallFunc:create(function(sender, event)
					if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
						...
						uiPanel_end:addChild(require("game.cdphz.48.GameEndLayer"):create(pBuffer))
					end
				end),
		...
		elseif subCmdID == NetMsgId.SUB_S_WD then
			if pBuffer.cbSubOperateCode == 2 then
				self.tableLayer:doAction(GameCommon.ACTION_WC, pBuffer)
				
			elseif pBuffer.cbSubOperateCode == 4 then
	...
	elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
		if subCmdID == NetMsgId.SUB_GF_SCENE then
			...
			GameCommon.bIsHuangZhuang = pBuffer.bIsHuangZhuang
			GameCommon.huangfanNum = pBuffer.cbHuangFanCount
			print('----重连:',GameCommon.huangfanNum)
			--设置臭偎和吃牌的顺序
			for i = 1, GameCommon.gameConfig.bPlayerCount do
			...
			if GameCommon.gameConfig.bPlayerCount == 4 and duijia == wChairID then
				self.tableLayer:setHandCard(GameCommon.wBankerUser, pBuffer.bUserCardCount[GameCommon.wBankerUser + 1], pBuffer.cbCardIndex, 8, 0, true)
...
function GameLayer:updatePlayerInfo()
	...
	for i = 1, GameCommon.gameConfig.bPlayerCount do
		...
		if GameCommon.bIsHuangZhuang == true and GameCommon.tableConfig.nTableType ~= TableType_GoldRoom then
			local logo ="cdzipai/ui/end_play_huangfan.png"
			uiImage_other:setVisible(true)
			uiImage_other:loadTexture(logo)
			uiImage_other:ignoreContentAdaptWithSize(true)
			local atlas_lab = uiImage_other:getChildByName('AtlasLabel_num')
			if GameCommon.huangfanNum == 0 or GameCommon.gameConfig.bHuangFanAddUp == 2 then
				uiImage_other:setVisible(false)
			end
			atlas_lab:setString('.' .. (GameCommon.huangfanNum + 1))
		else
		...
		if GameCommon.player == nil or GameCommon.player[wChairID] == nil then
			...
			--uiText_score:setString("循环次数:"..tostring(GameCommon.DGCS).."耗时："..tostring(GameCommon.timedata))
...
function GameLayer:updateBankerUser()
	for i = 1, GameCommon.gameConfig.bPlayerCount do
		...
		if GameCommon.player[wChairID] ~= nil and GameCommon.player[wChairID].wChairID == GameCommon.wBankerUser then
			uiImage_banker:loadTexture("cdzipai/ui/zhuang.png")
			local texture = cc.TextureCache:getInstance():addImage("cdzipai/ui/zhuang.png")
...
function GameLayer:updatePlayerHuXi(wChairID)
	...
	uiText_huXi:setString(string.format("胡息:%d", huXiCount))
end
	...
	for i = 1, GameCommon.gameConfig.bPlayerCount do
		...
		if GameCommon.player ~= nil and GameCommon.player[wChairID] ~= nil then
			...
			if GameCommon.player[wChairID].dwUserID == GameCommon.dwUserID and GameCommon.player[wChairID].bReady == true or GameCommon.gameState == GameCommon.GameState_Start then
				...
				self.tableLayer:refreshTablePos()
			end
		end	
	end
	--距离报警 
...
function GameLayer:updatePlayerOnline()
	...
	for i = 1, GameCommon.gameConfig.bPlayerCount do
		...
		if GameCommon.player ~= nil and GameCommon.player[wChairID] ~= nil then
			...
			local uiImage_avatar = ccui.Helper:seekWidgetByName(uiPanel_player, "Image_avatar")
			if GameCommon.player[wChairID].cbOnline == 6 then
...
function GameLayer:openDimissTable(params)
	...
	local path = self:requireClass('CDDisMissTable')
	local box = require("app.MyApp"):create(params):createGame(path)
	box:setName('CDDisMissTable')
	self:addChild(box)
end
...
function GameLayer:checkWhoAgree(data)
	if not data then
		return
	end
	local isAgree = false
