...
local TableLayer = require("game.zhuzhou.TableLayer")
...
function GameLayer:startGame(...)
    ...
    local csb = cc.CSLoader:createNode("GameLayerPengHuZhi.csb")
    self:addChild(csb)
    self.root = csb:getChildByName("Panel_root")
    self.csb = csb       
    GameCommon:init()
    GameCommon.wKindID = GameCommon.tableConfig.wKindID
    GameCommon.regionSound = cc.UserDefault:getInstance():getIntegerForKey('ZZvolumeSelect', 1) 
...
function GameLayer:openDimissTable( params )
    ...
    local path = self:requireClass('ZZDisMissTable')
    local box = require("app.MyApp"):create(params):createGame(path)
    box:setName('ZZDisMissTable')
    self:addChild(box)
end
...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    if mainCmdID == NetMsgId.MDM_GR_USER then   
       ...
       if subCmdID == NetMsgId.SUB_GR_USER_READY then
        ...
        elseif subCmdID == NetMsgId.SUB_GR_TABLE_STATUS then 
            ...
            uiText_des:setString(string.format("房间号:%d",roomId))
            if  GameCommon.tableConfig.nTableType > TableType_GoldRoom then      
                uiText_title:setString(string.format("局:%d/%d",randCeil,randFloor))  
            end 
            return true
        ...
        elseif subCmdID == NetMsgId.SUB_GR_DISMISS_TABLE_SUCCESS then
            if GameCommon.gameState ~= GameCommon.GameState_Init then
                ...
                require("common.MsgBoxLayer"):create(7,nil,"提示","房间解散成功！",function(sender,event) 
                    require("common.SceneMgr"):switchScene(require("app.MyApp"):create():createView("HallLayer"),SCENE_HALL) 
                end)   
            end
            return true
        ...
        elseif subCmdID == NetMsgId.SUB_GR_DISMISS_TABLE_STATE then
            ...
            if self:getChildByName('ZZDisMissTable') then
                self:getChildByName('ZZDisMissTable'):removeFromParent()
            end
            
            self:openDimissTable(data)
            self.tableLayer:disMissTableTips(data)
    ...
    elseif mainCmdID == NetMsgId.MDM_GR_LOGON then
        if subCmdID == NetMsgId.SUB_GR_LOGON_ERROR then
            ...
            require("common.MsgBoxLayer"):create(7,nil ,"提示", "您的金币不符" , function(sender,event) 
                require("common.SceneMgr"):switchScene(require("app.MyApp"):create():createView("HallLayer"),SCENE_HALL) 
            end)
            return true
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END then
            ...
            _tagMsg.pBuffer.lPengWeiTiPaoScore = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.lPengWeiTiPaoScore[i] = luaFunc:readRecvLong()--游戏积分
            end
            _tagMsg.pBuffer.lHuScore = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.lHuScore[i] = luaFunc:readRecvLong()--游戏积分
            end
            ...
            _tagMsg.pBuffer.HuCardInfo = {}
                for n = 1 , 4 do
                _tagMsg.pBuffer.HuCardInfo[n] = {}
                _tagMsg.pBuffer.HuCardInfo[n].cbCardEye = luaFunc:readRecvByte()--牌眼扑克
                _tagMsg.pBuffer.HuCardInfo[n].cbHuXiCount = luaFunc:readRecvByte()--胡息数目
                _tagMsg.pBuffer.HuCardInfo[n].cbWeaveCount = luaFunc:readRecvByte()--组合数目

                _tagMsg.pBuffer.HuCardInfo[n].WeaveItemArray = {}  --组合扑克
                for i = 1 , 7 do
                    _tagMsg.pBuffer.HuCardInfo[n].WeaveItemArray[i] = {}
                    _tagMsg.pBuffer.HuCardInfo[n].WeaveItemArray[i].cbWeaveKind = luaFunc:readRecvByte()--组合类型
                    _tagMsg.pBuffer.HuCardInfo[n].WeaveItemArray[i].cbCardCount = luaFunc:readRecvByte()--扑克数目
                    _tagMsg.pBuffer.HuCardInfo[n].WeaveItemArray[i].cbCenterCard = luaFunc:readRecvByte()--中心扑克
                    _tagMsg.pBuffer.HuCardInfo[n].WeaveItemArray[i].cbCardList = {}    --扑克列表
                    for j = 1 , 4 do
                        _tagMsg.pBuffer.HuCardInfo[n].WeaveItemArray[i].cbCardList[j] = luaFunc:readRecvByte()
                        print ("++++++WeaveItemArray++++++++",j,_tagMsg.pBuffer.HuCardInfo[n].WeaveItemArray[i].cbCardList[j])
                    end
                end
            end 
            -- _tagMsg.pBuffer.lUserScore = {}
            ...
            _tagMsg.pBuffer.cbHuType = luaFunc:readRecvDWORD()
            print("++++++++胡牌类型++++++++++",_tagMsg.pBuffer.cbHuType)
            GameCommon.wufu = {}
            self:updatePlayerInfo()
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    if mainCmdID == NetMsgId.MDM_GR_USER then   
        if subCmdID == NetMsgId.SUB_GR_GAME_STATISTICS then
            ...
            performWithDelay(self.root,function( ... )
                local path = self:requireClass('ZZGameRoomEnd')
                dump(pBuffer, 'OnGameRoomEnd::')
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START then
            ...
            self.tableLayer:updateLeftCardCount(80 - 4*14 - 1, true)
        ...
        elseif subCmdID == NetMsgId.SUB_S_USER_TI_CARD then
            ...
            self:showGoldView(pBuffer.lUserScore)
            self:updatePlayerInfo()
        elseif subCmdID == NetMsgId.SUB_S_USER_PAO_CARD then
...
--            end
            ...
            if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.zhuzhou.16.GameEndLayer"):create(pBuffer))
                    end
                    end),
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            --游戏重连
            for i = 1, GameCommon.gameConfig.bPlayerCount do 
                GameCommon.player[i-1].huXiCount = self.tableAddUserScore[i-1]
            end 
            self.tableLayer:updateGameState(GameCommon.GameState_Start)
            local wChairID = GameCommon:getRoleChairID()
            GameCommon.wBankerUser = pBuffer.wBankerUser
...
function GameLayer:showGoldView(lUserScore)
    local sChairID = -1
    local eChairID = -1
    for i=1,GameCommon.gameConfig.bPlayerCount do 
        if lUserScore[i] > 0 then 
            sChairID =  i - 1
        end       
    end      
    for i=1,GameCommon.gameConfig.bPlayerCount do 
       ...
       if lUserScore[i] < 0 then 
            eChairID =  i - 1  
            self.tableLayer:playGoldAnim(sChairID, eChairID)
        end  
    end
end
...
function GameLayer:showMoneyView(wChairID,money)--显示输赢
    ...
    if money > 0 then
        GameCommon.player[wChairID].huXiCount = GameCommon.player[wChairID].huXiCount + money
...
function GameLayer:updatePlayerInfo()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.player == nil or GameCommon.player[wChairID] == nil then
            ...
            local szGold = Common:itemNumberToString(GameCommon.player[wChairID].lScore)
            if GameCommon.tableConfig.nTableType ~= TableType_GoldRoom then
                uiText_score:setString(string.format("总胡:%s",szGold))  
                uiText_huXi:setString(string.format("%d胡",GameCommon.player[wChairID].huXiCount))      
            else
                uiText_score:setString(string.format("金币:%s",szGold))   
                uiText_huXi:setString(string.format("%d",GameCommon.player[wChairID].huXiCount))     
            end       
        end
        ...
        if  GameCommon.wufu[viewID]~=nil then            
            if GameCommon.wufu[viewID] == true  then-- GameCommon.wufu[viewID] == viewID and .add
                ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("zhuzhou/animation/wufubaojing-texiao2/wufubaojing-texiao.ExportJson")
                ...
                armature:setTag(1001)
                    if viewID ~= 1 then
                        ...
                        armature:setPosition(cc.p(540+armature:getParent():getContentSize().width/2,armature:getParent():getContentSize().height/2+80))
                    end         
            end
        end 
    end       
...
function GameLayer:updateBankerUser()
    for i = 1 , GameCommon.gameConfig.bPlayerCount do      
        ...
        if GameCommon.player[wChairID] ~= nil and GameCommon.player[wChairID].wChairID == GameCommon.wBankerUser then
            print("++++++++++++++++++装：",GameCommon.wBankerUser,GameCommon.player[wChairID].wChairID)                
           ...
           if GameCommon.wContinueWinCount < 1 then 
                ...
                uiImage_banker:loadTexture(textureName)
                ...
                if GameCommon.tableConfig.bSuccessive == 0 then
                    ...
                    uiImage_banker:addChild(uiText_LZ)
