...
function PersonalLayer:create(dwUserID,dwShamUserID,dwUserip)
    local view = PersonalLayer.new()
    view:onCreate(dwUserID,dwShamUserID,dwUserip)
...
function PersonalLayer:onCreate(dwUserID,dwShamUserID,dwUserip)
    local visibleSize = cc.Director:getInstance():getVisibleSize()
    local csb = cc.CSLoader:createNode("YZUserInfoLayer.csb")
    self:addChild(csb)
    ...
    local visibleSize = cc.Director:getInstance():getVisibleSize()
    local uiImage_avatarBg = ccui.Helper:seekWidgetByName(self.root,"Image_avatarBg")
    Common:requestUserAvatar(player.dwUserID,player.szPto,uiImage_avatarBg,"img")
    ...
    Common:addTouchEventListener(uiButton_copy,function()   
        local btnName = string.format("%d", player.dwUserID)
        print(player.dwUserID,btnName)     
        UserData.User:copydata(btnName)
        require("common.MsgBoxLayer"):create(0,nil,"复制成功")
    end)
    local Userip = NetMgr:getLogicInstance().cppFunc:int2ip(dwUserip)
    local uiText_ip = ccui.Helper:seekWidgetByName(self.root,"Text_ip")
    uiText_ip:setString(string.format("%s",Userip))
...
local AnimCnf = {
    {'yongzhou/info/quantao', 'yongzhou/anim/baodai'},
    {'yongzhou/info/daoshui', 'yongzhou/anim/daoshui'},
    {'yongzhou/info/fq', 'yongzhou/anim/fq'},
    {'yongzhou/info/yiduohua', 'yongzhou/anim/meigui008'},
    {'yongzhou/info/pengbei', 'yongzhou/anim/pengbei'},
    {'yongzhou/info/yibahua', 'yongzhou/anim/xianhua'},
}
...
function PersonalLayer:setFaceActions(dwUserID)
    ...
    for i,v in ipairs(AnimCnf) do
        ...
        item:addClickEventListener(function()
            ...
            if targetChair  then--and  self.isOpen == true
