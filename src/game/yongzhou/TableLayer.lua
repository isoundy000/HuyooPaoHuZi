...
local SettingsLayer = require("game.yongzhou.SettingsLayer")
local GameLogic = require("game.yongzhou.GameLogic")
local UserData = require("app.user.UserData")
local GameDesc = require("common.GameDesc")
local Log      = require("common.Log")
local PersonalLayer = require("game.yongzhou.PersonalLayer")
...
function TableLayer:doAction(action,pBuffer)
    ...
    if action == GameCommon.ACTION_OUT_CARD_NOTIFY then
        ...
        if wChairID == GameCommon:getRoleChairID() then
         ...
         --   if GameCommon.outOprationState then
         ...
         --   end            
            ...
            ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("yongzhou/anim/finger/finger.ExportJson")
    ...
    elseif action == GameCommon.ACTION_CHI_CARD then
        ...
        for i = 1, pBuffer.cbResultCount do
            ...
            if uiSendOrOutCardNode ~= nil then
                self:addWeaveItemArray(wChairID, WeaveItemArray, GameCommon.player[wChairID].bWeaveItemCount + 1,cc.p(uiSendOrOutCardNode:getPosition()),i)
            else
                self:addWeaveItemArray(wChairID, WeaveItemArray, GameCommon.player[wChairID].bWeaveItemCount + 1,cc.p(uiPanel_tipsCardPos:getPosition()),i)
            end
    ...
    elseif action == GameCommon.ACTION_OPERATE_NOTIFY then
        if pBuffer.cbOperateCode ~= GameCommon.ACK_NULL then
            --托管
          --  if GameCommon.outOprationState == true then
          --      GameOperation:create():Tuoguan_Status(pBuffer)
          --  else      
...
function TableLayer:showLeftCardCount(bLeftCardCount, bLeftCardData)
    ...
    uiListView_showStack1:setVisible(false)
    uiListView_showStack2:setVisible(false)
    local cardScale = 1
    local time = 0.1
...
function TableLayer:updateLeftCardCount(bLeftCardCount, isEffects, isSendCardEffects)
    ...
    for i = 1, showCount do
        local img = nil
        img = ccui.ImageView:create("yongzhou/card/fapai_pai.png")
...
--添加吃牌组合
function TableLayer:addWeaveItemArray(wChairID,WeaveItemArray,location, pos,num)
    GameCommon.player[wChairID].bWeaveItemCount = GameCommon.player[wChairID].bWeaveItemCount + 1
    table.insert(GameCommon.player[wChairID].WeaveItemArray, location, WeaveItemArray)
    local viewID = GameCommon:getViewIDByChairID(wChairID)
    local node = self:setWeaveItemArray(wChairID, GameCommon.player[wChairID].bWeaveItemCount, GameCommon.player[wChairID].WeaveItemArray,location,num)
    if num == nil   then 
        num = 1
    end
    if pos ~= nil then
        ...
        local uiPanel_tipsCardPos = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_tipsCardPos%d",viewID))
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        print(num,uiPanel_tipsCardPos:getPositionX()+(num-1)*100)
        node:setPosition(cc.p(node:getParent():convertToNodeSpace(cc.p(uiPanel_tipsCardPos:getPositionX()+(num-1)*100,uiPanel_tipsCardPos:getPositionY()))))
        node:runAction(cc.Sequence:create(cc.ScaleTo:create(0.1, 1.6,2.0),cc.DelayTime:create(0.2),
                    cc.Spawn:create(cc.ScaleTo:create(0.2, 1.0),cc.MoveTo:create(0.2,srcPos))))
    end
end

--更新吃牌组合
function TableLayer:setWeaveItemArray(wChairID, bWeaveItemCount, WeaveItemArray,location,num)
...
--更新吃牌组合
function TableLayer:setWeaveItemlargeArray(wChairID, bWeaveItemCount, WeaveItemArray,location)
    GameCommon.player[wChairID].bWeaveItemCount = bWeaveItemCount
    GameCommon.player[wChairID].WeaveItemArray = WeaveItemArray
    local viewID = GameCommon:getViewIDByChairID(wChairID)
    local uiPanel_weaveItemArray = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_weaveItemArray%d",viewID))
...
function TableLayer:showHandCard(wChairID,effectsType,isShowEndCard)
    ...
    if isShowEndCard == true then
        ...
        uiPanel_handCard = ccui.Helper:seekWidgetByName(GameCommon.uiPanel_showEndCard,string.format("Panel_handCard%d",viewID))
        local uiPanel_tipsCardPos = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_tipsCardPos%d",viewID))
        cardScale = 1
        if viewID == 1 and (GameCommon.gameConfig.bPlayerCount <= 3 or (GameCommon.gameConfig.bPlayerCount == 4 and StaticData.Games[GameCommon.tableConfig.wKindID].isZuoXing4 == 1))  then
            uiPanel_handCard = ccui.Helper:seekWidgetByName(GameCommon.uiPanel_showEndCard,"Panel_handCardRole")
        end
        if viewID == 1  and GameCommon.gameConfig.bPlayerCount == 4  then 
            cardScale = 0.6
        end
    else
        local uiPanel_card = ccui.Helper:seekWidgetByName(self.root,"Panel_card")
    ...
    if viewID~= 1  then 
        index = 0
        time = 0.1
        cardWidth = 42 * cardScale
        cardHeight = 42 * cardScale
        stepX = cardWidth
        stepY = cardHeight * 1
        beganX = (size.width - nCardStackCount * stepX) / 2
    elseif viewID == 1 and GameCommon.gameConfig.bPlayerCount == 4 and StaticData.Games[GameCommon.tableConfig.wKindID].isZuoXing4 ~= 1 
    and GameCommon.tableConfig.nTableType == TableType_Playback then 
        index = 0
        time = 0.1
        cardWidth = 42 * cardScale
        cardHeight = 42 * cardScale
    ...
    for key, var in pairs(cardStackInfo) do
        for k, v in pairs(var.cbCardData) do
            ...
            if viewID~= 1 then 
                card = GameCommon:getDiscardCardAndWeaveItemArray(v.data)
            elseif viewID == 1 and GameCommon.gameConfig.bPlayerCount == 4 and StaticData.Games[GameCommon.tableConfig.wKindID].isZuoXing4 ~= 1 
            and GameCommon.tableConfig.nTableType == TableType_Playback then 
                card = GameCommon:getDiscardCardAndWeaveItemArray(v.data)
            end
...
--                card:runAction(cc.Sequence:create(cc.DelayTime:create(1*index*0.03),cc.Spawn:create(cc.ScaleTo:create(time,cardScale),cc.MoveTo:create(time,v.pt))))
                if anchorPoint.x == 0.5 then
                    card:setPosition(uiPanel_handCard:getContentSize().width/2, stepY*(k-1) + beganY)
                elseif anchorPoint.x == 0 then
                    card:setPosition(beganX + 1*stepX - cardWidth/2, stepY*(k-1) + beganY)
                else
                    card:setPosition(beganX + nCardStackCount*stepX - cardWidth/2, stepY*(k-1) + beganY)
                end
                v.pt = cc.p(beganX + key*stepX - cardWidth/2, stepY*(k-1) + beganY)
                card:setScale(cardScale)--math.abs(v.pt.y - card:getPositionY())*time
                card:runAction(cc.MoveTo:create(k*0.15,v.pt))
                    ...
                    elseif event == ccui.TouchEventType.moved then
                        ...
                        if self.copyHandCard ~= nil then
                            ...
                            elseif row > nCardStackCount then   --插入最右边
                                ...
                                if (GameCommon.player[wChairID].cardStackInfo[row].nCardCount < 4) or
...
function TableLayer:initUI()
  ...
  --  uiImage_watermark:loadTexture(StaticData.Channels[CHANNEL_ID].icon)
    ...
    Common:addTouchEventListener(ccui.Helper:seekWidgetByName(self.root,"Button_settings"),function()       
        require("game.yongzhou.SettingsLayer"):create()
    end)
    ...
    local function onEventExpression(sender,event)
        if event == ccui.TouchEventType.ended then
            Common:palyButton()
            require("game.yongzhou.GameChatLayer"):create(GameCommon.tableConfig.wKindID,function(index) 
    ...
    Common:addTouchEventListener(uiButton_disbanded,function() 
        require("game.yongzhou.MsgBoxLayer"):create(1,nil,"是否确定解散房间？",function()
    ...
    -- local uiButton_hosted = ccui.Helper:seekWidgetByName(self.root,"Button_hosted")  --托管
    -- Common:addTouchEventListener(uiButton_cancel,function() 
    --     self:startOutOpration()
    -- end) 
    -- uiButton_hosted:setVisible(false)
    ...
    local uiPanel_playerInfoBg4 = ccui.Helper:seekWidgetByName(self.root,"Panel_playerInfoBg4")
    local uiPanel_playerInfoBg3 = ccui.Helper:seekWidgetByName(self.root,"Panel_playerInfoBg3")
    ...
    if GameCommon.tableConfig.wCurrentNumber == 0 and  GameCommon.tableConfig.nTableType > TableType_GoldRoom then
        if CHANNEL_ID ~= 0 and CHANNEL_ID ~= 1 then
            uiPanel_playerInfoBg4:setVisible(true) 
            uiPanel_playerInfoBg3:setVisible(true) 
        else 
            uiPanel_playerInfoBg4:setVisible(false)
            uiPanel_playerInfoBg3:setVisible(false)
        end
    end
    ...
    if GameCommon.tableConfig.nTableType > TableType_GoldRoom then
        ...
        elseif GameCommon.tableConfig.wCurrentNumber > 0 then
            ...
            ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("yongzhou/anim/paohuzi_dengren/paohuzi_dengren.ExportJson")
            local waitArmature=ccs.Armature:create("paohuzi_dengren")
            waitArmature:getAnimation():playWithIndex(1)
            uiPanel_ready:addChild(waitArmature)            
            waitArmature:setPosition(680,320)
            NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GR_USER,NetMsgId.REQ_GR_USER_READY,"o",false) 
            --安全处理，保证下局开始前清理上局脏数据
           -- GameCommon.reconnectCardInfo = {}
        end
        if StaticData.Hide[CHANNEL_ID].btn4 ~= 1 then
            uiButton_Invitation:setVisible(false)
            uiButton_ready:setPositionX(visibleSize.width*0.5)   
        else
            uiButton_Invitation:setVisible(true)
        end
        uiText_des:setString(string.format("房间号:%d \n局数: %d/%d",GameCommon.tableConfig.wTbaleID, GameCommon.tableConfig.wCurrentNumber, GameCommon.tableConfig.wTableNumber))   
    ...
    elseif GameCommon.tableConfig.nTableType == TableType_GoldRoom then            
        ...
        uiButton_out:setVisible(false)
        uiButton_disbanded:setVisible(false)
        -- uiButton_hosted:setVisible(true)
        local uiButton_position = ccui.Helper:seekWidgetByName(self.root,"Button_position")   -- 定位
...
function TableLayer:addVoice()
    ...
    local uiButton_voice = ccui.Helper:seekWidgetByName(self.root,"Button_voice")
    local animVoice = cc.CSLoader:createNode("YZVoiceNode.csb")
    ...
    uiText_surplus:setVisible(false)
    local uiText_tips = ccui.Helper:seekWidgetByName(root,"Text_tips")
    uiText_tips:setVisible(false)
    ...
    local function onEventVoice(sender,event)
        if event == ccui.TouchEventType.began then
            ...
            local node = require("common.CircleLoadingBar"):create("yongzhou/ui/yongzhou_yuying02.png")
            local uiImage_pro = ccui.Helper:seekWidgetByName(root,"Image_pro")
            uiImage_pro:removeAllChildren()
    ...
    local function onEventPlayVoice(sender,event)
        if #self.tableVoice > 0 then
            ...
            print(">>>>>>>>>>>>>>",data.time,math.floor(data.time*2))
            ...
            local uiPanel_chat = ccui.Helper:seekWidgetByName(wanjia,"Panel_chat")
            local blinks = math.floor(data.time*2)+1


            ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("yongzhou/anim/yuyinlaba/yuyinlaba.ExportJson")
            local waitArmature=ccs.Armature:create("yuyinlaba")
            waitArmature:setPosition(0,0)            
            waitArmature:getAnimation():playWithIndex(0)
            uiPanel_chat:addChild(waitArmature)
            waitArmature:runAction(cc.Sequence:create(               
                cc.CallFunc:create(function(sender,event) 
...
function TableLayer:showPlayerPosition()   -- 显示玩家距离
    ...
    if GameCommon.gameConfig.bPlayerCount == 4 then
        uiPanel_playerInfoBg4:setVisible(true)
        uiPanel_playerInfoBg = uiPanel_playerInfoBg4   
    else        
        uiPanel_playerInfoBg3:setVisible(true)
        uiPanel_playerInfoBg = uiPanel_playerInfoBg3
    end  
    for wChairID = 1, GameCommon.gameConfig.bPlayerCount do
        local uiPanel_players = ccui.Helper:seekWidgetByName(uiPanel_playerInfoBg,string.format("Panel_players%d",wChairID))
    ...
    for wChairID = 0, num do   
        local viewID = GameCommon:getViewIDByChairID(wChairID)
        local uiPanel_players = ccui.Helper:seekWidgetByName(uiPanel_playerInfoBg,string.format("Panel_players%d",viewID))
...
function TableLayer:showPlayerInfo(dwUserID,dwShamUserID,dwUserip)       -- 查看玩家信息
    Common:palyButton()
    PersonalLayer:create(dwUserID,dwShamUserID,dwUserip)
end
...
function TableLayer:showChat(pBuffer)
    ...
    local wKindID = GameCommon.tableConfig.wKindID
--   Chat = require("common.Chat")
    local Chat = require("game.yongzhou.Animation")["Chat"]
...
function TableLayer:showExperssion(pBuffer)
	local viewID = GameCommon:getViewIDByChairID(pBuffer.wChairID)
        local uiPanel_play = ccui.Helper:seekWidgetByName(self.root,"Panel_player")
        local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_player%d",viewID))
        local uiPanel_chat  = ccui.Helper:seekWidgetByName(uiPanel_player,"Panel_chat")
        local filename = ""
        local num = pBuffer.wIndex +1         
        require("common.Common"):playEffect(string.format("expression/sound/%s.mp3",filename))
        local sCircle = cc.Sprite:create(string.format("yongzhou/chat/%d.png",num))
        uiPanel_chat:addChild(sCircle)
        sCircle:setPosition(0,0)
        local pos = 30 
        if viewID == 3 or viewID == 4 then
            pos = -30  
        end 
        sCircle:runAction(cc.Sequence:create(cc.MoveTo:create(0.2,cc.p(pos,0)),cc.MoveTo:create(0.2,cc.p(0,0)),cc.FadeOut:create(0.5),cc.RemoveSelf:create()))
...
--==============================--
--desc:开始托管与退出托管
--time:2018-09-20 04:25:32
--@return 
--==============================--
function TableLayer:startOutOpration()

    --self.outOprationLayOut:setVisible(true)
    GameCommon.outOprationState =true
...
function TableLayer:overOutOpration()
    -- self.outOprationIndex=0
    GameCommon.outOprationState =false
    --self.outOprationLayOut:setVisible(false)
end


function TableLayer:OnUserAutoOutCard()
    self:showHandCard(GameCommon:getRoleChairID(),0)

    local cardStackInfo = GameCommon.player[GameCommon:getRoleChairID()].cardStackInfo
    --寻找单牌
    local card = GameLogic:GetBestOutCard(cardStackInfo,0)
