...
function ShareLayer:onCreate(params)
    ...
    local uiText_jiehsao = ccui.Helper:seekWidgetByName(self.root,"Text_jiehsao")
    uiText_jiehsao:retain()
    uiText_jiehsao:setVisible(false)
        ...
        if Bit:_and((Bit:_rshift(shareData.cbTargetType,i)),1) == 1 then
            ...
            if i ~= 3 then
                if i == 6 and (not isInClub) then--不在俱乐部不显示6
                    ...
                    local uitext = nil 
                    if i == 0 then 
                        uitext = ccui.Text:create("朋友圈","fonts/DFYuanW7-GB2312.ttf",24)
                    elseif i == 1 then 
                        uitext = ccui.Text:create("微信好友","fonts/DFYuanW7-GB2312.ttf",24)
                    elseif i == 2 then 
                        uitext = ccui.Text:create("闲聊","fonts/DFYuanW7-GB2312.ttf",24)
                    elseif i == 3 then 
                        uitext = ccui.Text:create("聊天室","fonts/DFYuanW7-GB2312.ttf",24)
                    elseif i == 4 then 
                        uitext = ccui.Text:create("战绩链接","fonts/DFYuanW7-GB2312.ttf",24)
                    elseif i == 5 then
                        uitext = ccui.Text:create("邀请在线好友","fonts/DFYuanW7-GB2312.ttf",24)
                    elseif i == 6 then
                        uitext = ccui.Text:create("聊天室","fonts/DFYuanW7-GB2312.ttf",24)
                    end 
