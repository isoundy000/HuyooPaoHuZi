...
function LoginLayer:onCreate(parames)
    ...
    for i = 1 , 7 do
        local userAnim = ccui.Helper:seekWidgetByName(self.root, string.format("Imagexin_%d", i))   
        local function RunXiXi()
            userAnim:runAction(cc.Sequence:create(cc.DelayTime:create(0.1*(i-1)),cc.ScaleTo:create(0.1*(i+1),1.5),cc.ScaleTo:create(0.1*(7-i),0.2)
    ...
    -- local skeletonNode = sp.SkeletonAnimation:create('login/huaping/huaping.json','login/huaping/huaping.atlas')
    -- self.root:addChild(skeletonNode)
    -- skeletonNode:setName('animation')
    -- skeletonNode:setPosition(visibleSize.width - 61 ,0)
